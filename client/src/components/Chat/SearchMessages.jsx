    ...
    <div className="overflow-auto custom-scrollbar h-full">
      ...
      <div className="flex flex-col w-full h-full">
        {searchedMessages.map((message, i) => (
          <div key={i} className="flex cursor-pointer flex-col justify-center hover:bg-background-default-hover w-full px-5 border-b-[0.1px] border-secondary py-5">
              <div className="text-sm text-secondary">{calculateTime(message.createdAt)}</div>
              <div className="text-icon-green">{message.messages}</div>
            </div>
        ))}
        {searchTerm && searchedMessages.length > 0 && (
<div className="text-secondary px-5 py-2">{`Found ${searchedMessages.length} messages`}</div>
)} 
      </div>
      </div>
    </div>
